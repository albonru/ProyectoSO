//servidor.c para la version 5 con: cross-threading y chat
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <mysql.h>
...
void InicializarTablaPartidas (TablaPartidas tabla) {
	//los sockets de todas las partidas inicializados a -1
	for (int i=0; i<40;i++){
		tabla[i].sock1 = -1;
		tabla[i].sock2 = -1;
	}
...
{
	//ocupa la primera posicion libre en lista de partidas
...
void EliminarPartida (int IDpartida) {
	//vuelve a poner los sockets de una partida a -1
...
void Invitar (int socket, char user2[35]) {
	//crea partida y manda invitacion del tipo 7/anfition/#partida
...
void RespuestaInvitacion(int IDpartida, char invRespuesta[5]) {
	...
	//formato tipo 9/respuesta/#partida
...
void Mensaje (int IDpartida, char mensajeRecibido[200], int sock_conn) {
	//chat entre dos jugadores de una misma partida
	//pasamos el mensaje como notificacion
	printf("Mensaje recibido: %s\nDesde el socket: %d\n",mensajeRecibido,sock_conn);
	char notificacion[512];

	if(sock_conn == tabla[IDpartida].sock1)
		sprintf (notificacion, "11/%d/%s: %s", IDpartida,tabla[IDpartida].user1,mensajeRecibido);
	else if(sock_conn == tabla[IDpartida].sock2) 
		sprintf (notificacion, "11/%d/%s: %s", IDpartida,tabla[IDpartida].user2,mensajeRecibido);
		
	write (tabla[IDpartida].sock2, notificacion, strlen(notificacion));
	printf("Notificacion %s enviada hacia el socket2: %d\n",notificacion,tabla[IDpartida].sock2);
	write (tabla[IDpartida].sock1, notificacion, strlen(notificacion));
	printf("Notificacion %s enviada hacia el socket1: %d\n",notificacion,tabla[IDpartida].sock1);
}
...
{
	//carga usuario en la base de datos
...
{
	//si el logn es correcto se verifica que se pueda anyadir a la lista de conectados
...
void *AtenderCliente (void *socket)
	//maneja el thread de un cliente y todas sus peticiones hasta deconexion
{
	int sock_conn;
	int *s;
	s= (int *) socket;
